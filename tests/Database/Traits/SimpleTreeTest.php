...
use Winter\Storm\Tests\Database\Fixtures\Models\CategorySimpleTree;
...
class SimpleTreeTest extends DbTestCase
{
    ...
    {
        $items = CategorySimpleTree::getNested();
    ...
    {
        $items = CategorySimpleTree::getAllRoot();
    ...
    {
        // Not eager loaded
        $item = CategorySimpleTree::first();
        $this->assertFalse($item->relationLoaded('children'));
        $this->assertEquals(6, $item->getChildren()->count());

        // Not eager loaded
        $item = CategorySimpleTree::getAllRoot()->first();
        $this->assertFalse($item->relationLoaded('children'));
        $this->assertEquals(6, $item->getChildren()->count());

        // Eager loaded
        $item = CategorySimpleTree::getNested()->first();
        $this->assertTrue($item->relationLoaded('children'));
        $this->assertEquals(6, $item->getChildren()->count());
    }
    ...
    {
        $array = CategorySimpleTree::listsNested('name', 'id');
        $this->assertEquals([
            1 => 'Web development',
            2 => '&nbsp;&nbsp;&nbsp;HTML5',
            3 => '&nbsp;&nbsp;&nbsp;CSS3',
        ...
        $array = CategorySimpleTree::listsNested('id', 'name', '**');
        $this->assertEquals([
            'Web development' => '1',
            'HTML5' => '**2',
            'CSS3' => '**3',
    ...
    public function testListsNestedUnknownColumn()
    {
        ...
        CategorySimpleTree::listsNested('custom_name', 'id');
    }

    public function testListsNestedFromCollection()
    {
        $array = CategorySimpleTree::get()->listsNested('custom_name', 'id', '...');
    ...
    public function seedSampleTree()
    {
        ...
        $webdev = CategorySimpleTree::create([
        ...
        $mobdev = CategorySimpleTree::create([
        ...
        $design = CategorySimpleTree::create([
            'name' => 'Graphic design'
        ]);
