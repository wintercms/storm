...
class MarkdownTest extends TestCase
{
    ...
    {
        $this->assertEquals($html, static::removeLineEndings(Markdown::parse($markdown)), 'Markdown test case "' . $name . '" failed');
    }

    public function testParseSafe()
    {
        $markdown = file_get_contents(dirname(__DIR__) . '/fixtures/markdown/code_block.md');
        $html = static::removeLineEndings(file_get_contents(dirname(__DIR__) . '/fixtures/markdown/code_block_disabled.html'));

        $this->assertEquals($html, static::removeLineEndings(Markdown::parseSafe($markdown)));
    }
    ...
    public function testDisableAndEnableMethods()
    {
        ...
        $html = static::removeLineEndings(file_get_contents(dirname(__DIR__) . '/fixtures/markdown/table_inline_markdown_disabled.html'));

        $this->assertEquals($html, static::removeLineEndings($parser->parse($markdown)));

        // Re-enable tables and autolinking
        $parser->enableAutolinking()->enableTables();

        $html = static::removeLineEndings(file_get_contents(dirname(__DIR__) . '/fixtures/markdown/table_inline_markdown.html'));
    ...
    public function testFrontMatter()
    {
        ...
        $html = static::removeLineEndings(file_get_contents(dirname(__DIR__) . '/fixtures/markdown/front_matter.html'));
    ...
    public static function markdownData()
    {
        ...
        foreach ($dir as $file) {
            if ($file->isFile() && $file->getExtension() === 'md') {
                ...
                if (in_array($name, static::$specialTests)) {
                    continue;
                }

                $markdown = file_get_contents($dirName . DIRECTORY_SEPARATOR . $name . '.md');
                $html = static::removeLineEndings(file_get_contents($dirName . DIRECTORY_SEPARATOR . $name . '.html'));
